...
{
    /// <summary>
    /// Renders an image on the screen with a post-processing effect.
    /// </summary>
    public class OverlayImagePostProcessPass : ScriptableRenderPass
    {
        ...
        protected Material PostProcessMaterial
        {
            get
            {
                ...
                {
                    m_postProcessMaterial = new Material(Shader.Find("Game/S_OverlayImagePostProcess"));
                }
        ...
        const string k_RenderPostProcessingTag = "Render Overlay Image PostProcessing Effect";
        ...
        public OverlayImagePostProcessPass(RenderPassEvent evt, PostProcessData data, Material blitMaterial)
        {
        ...
        {
            OverlayImage overlayImage = VolumeManager.instance.stack.GetComponent<OverlayImage>();
            ...
            if (overlayImage.IsActive() && !cameraData.isSceneViewCamera)
            {
                ...
                PostProcessMaterial.SetFloat("_AlphaPower", overlayImage.AlphaPower.value);
                PostProcessMaterial.SetFloat("_AlphaOffset", overlayImage.AlphaOffset.value);
                PostProcessMaterial.SetFloat("_InvertAlpha", overlayImage.InvertAlpha.value ? 1.0f : 0.0f);
                PostProcessMaterial.SetColor("_ImageColor", overlayImage.ImageColor.value);
                PostProcessMaterial.SetTexture("_Image", overlayImage.Image.value);
                PostProcessMaterial.SetFloat("_TextureAlphaMinimumClipThreshold", overlayImage.TextureAlphaMinimumClipThreshold.value);
                PostProcessMaterial.SetFloat("_TextureAlphaMaximumClipThreshold", overlayImage.TextureAlphaMaximumClipThreshold.value);
